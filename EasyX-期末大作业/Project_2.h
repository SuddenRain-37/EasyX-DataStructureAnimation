...
protected:
	...
	std::wstring getAlgorithmTitle() const override { return L"二叉树中序遍历"; }
	std::wstring getAlgorithmInfo() const override {
		return L"中序遍历是二叉树遍历的一种方式，遍历顺序为：左子树 → 根节点 → 右子树。"
		       L"对于二叉搜索树，中序遍历会得到一个升序序列。本动画演示了二叉树 [50,25,75,10,30,60] 的中序遍历过程。"
		       L"遍历结果应为：10, 25, 30, 50, 60, 75。绿色表示已访问的节点，黄色表示当前正在处理的节点。";
	}
...
private:
	...
	void initAnimation()
	{
		...
		// ========== 生成中序遍历步骤 ==========
		// 中序遍历：左 -> 根 -> 右
		// 结果应为：10, 25, 30, 50, 60, 75
		generateInorderSteps(0);
	...
	void generateInorderSteps(int nodeIdx)
	{
		if (nodeIdx < 0 || nodeIdx >= (int)treeNodes.size()) return;

		// 1. 访问左子树前先标记当前节点（准备进入左子树）
		traversalSteps.push_back({ nodeIdx, -1,
			L"到达节点 " + std::to_wstring(treeNodes[nodeIdx].value) + L"，准备遍历左子树" });

		// 2. 遍历左子树
		if (treeNodes[nodeIdx].leftChild >= 0) {
			generateInorderSteps(treeNodes[nodeIdx].leftChild);
		}
		...
		if (treeNodes[nodeIdx].rightChild >= 0) {
			generateInorderSteps(treeNodes[nodeIdx].rightChild);
		}
	}
	...
	void drawTraversalResult() const
	{
		...
		setbkmode(TRANSPARENT);
		std::wstring label = L"遍历顺序（中序：左 → 根 → 右）：";
	...
	void drawStepInfo() const
	{
		std::wstring info;
		if (currentStep < 0) {
			info = L"点击“下一步”开始中序遍历演示";
		}
		else if (currentStep >= (int)traversalSteps.size()) {
			info = L"遍历完成！结果：10, 25, 30, 50, 60, 75";
		}
		else {
