...
protected:
	...
	std::wstring getAlgorithmTitle() const override { return L"二叉树后序遍历"; }
	std::wstring getAlgorithmInfo() const override {
		return L"后序遍历是二叉树遍历的一种方式，遍历顺序为：左子树 → 右子树 → 根节点。"
		       L"后序遍历常用于删除二叉树（先删除子节点再删除父节点）或计算表达式的值。本动画演示了二叉树 [50,25,75,10,30,60] 的后序遍历过程。"
		       L"遍历结果应为：10, 30, 25, 60, 75, 50。绿色表示已访问的节点，黄色表示准备遍历子树的节点。";
	}
...
private:
	...
	void initAnimation()
	{
		...
		// ========== 生成后序遍历步骤 ==========
		// 后序遍历：左 -> 右 -> 根
		// 结果应为：10, 30, 25, 60, 75, 50
		generatePostorderSteps(0);
	...
	void generatePostorderSteps(int nodeIdx)
	{
		...
		if (treeNodes[nodeIdx].leftChild >= 0) {
			traversalSteps.push_back({ nodeIdx, -1,
				L"节点 " + std::to_wstring(treeNodes[nodeIdx].value) + L" 准备遍历左子树" });
			generatePostorderSteps(treeNodes[nodeIdx].leftChild);
		}
		...
		if (treeNodes[nodeIdx].rightChild >= 0) {
			...
			generatePostorderSteps(treeNodes[nodeIdx].rightChild);
		}

		// 3. 访问当前节点（记录到遍历顺序中）—— 后序遍历的核心
	...
	void drawTraversalResult() const
	{
		...
		setbkmode(TRANSPARENT);
		std::wstring label = L"遍历顺序（后序：左 → 右 → 根）：";
	...
	void drawStepInfo() const
	{
		std::wstring info;
		if (currentStep < 0) {
			info = L"点击“下一步”开始后序遍历演示";
		}
		else if (currentStep >= (int)traversalSteps.size()) {
			info = L"遍历完成！结果：10, 30, 25, 60, 75, 50";
		}
		else {
